...
import { ProviderSDK as ProviderSDKBase } from '@lynx-sdk/react'
...
const ProviderSDK: FC<ProviderWeb3Props> = props => {
  const { rpc, defaultChainId, supportedChainIds, children, pollingInterval = POLLING_INTERVAL, ...rest } = props
  const { active: isConnectedViaWeb3React, library } = useWeb3React()
  const { chainId = defaultChainId, account, active } = useWeb3()
  const { connector: connectorWagmi, isConnected: isConnectedViaWagmi } = useAccount()
  ...
  const providerRpc = getStaticRpcBatchProvider(chainId, rpc[chainId], 0, pollingInterval)

  const providerMainnetRpc = getStaticRpcBatchProvider(CHAINS.Mainnet, rpc[CHAINS.Mainnet], 0, pollingInterval)
...
const ProviderWeb3: FC<ProviderWeb3Props> = props => {
  const { children, rpc, walletconnectProjectId, appName, appLogoUrl, ...sdkProps } = props
  ...
  const supportedWagmiChains = wagmiChainsArray.filter(chain => supportedChainIds.includes(chain.id))
  const defaultWagmiChain = wagmiChainsArray.find(chain => chain.id === defaultChainId)
